...
### Recommended fast-follows (Minor, non-blocking — from the final integration review)
- **reindex read via shared gate (Low/hardening):** `packages/core/src/memory/reindex.ts` `parseMemoryNote` reads via a bare `readFileSync(absPath)`. It IS zone-gated (a `resolveZone(...) === "excluded"` check `continue`s before the read), but it bypasses `assertContainedAndReadable`'s lexical/symlink layer. Safe today (`absPath` comes from the internal directory walk, never external input), but inconsistent with the batch's "single shared gate" principle. Route it through the shared helper for defense-in-depth. Expiry: close before any change lets a caller-supplied `agentGlobs`/path reach `parseMemoryNote` without the walk.
- **De-duplicate `loadContext`/`openVault` wiring (hygiene, pre-existing):** the two near-identical broker/store/approvals construction sites should be one function — a future required-dependency change could be applied to only one without a compiler catch (the `lockDir`-required change happened to be TS-caught here). Not new debt from this batch.
