...
// EXISTS at the very first `pnpm install`, so pnpm links the `ledger` bin
// immediately — unlike a `"bin": "./dist/index.js"` field, which doesn't
// exist yet at first install and so links nothing (pnpm 11's second
// `pnpm install` is a no-op, it does not retroactively create the link).
//
